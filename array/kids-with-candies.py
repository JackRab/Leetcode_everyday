...
class Solution:
    def kidsWithCandies(self, candies: List[int], extraCandies: int) -> List[bool]:
        """
        Return a list of whether a kid can have the greatest number of candies if given extra candies
        """
        """
        the idea is to find the maximum first, then compare if candies[i] + extra is larger than or equal to max
        """
        """
        Time complexity: max O(N), get res O(N), total O(N)
