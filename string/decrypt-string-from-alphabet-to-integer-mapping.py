...
class Solution:
    def freqAlphabets(self, s: str) -> str:
        ...
        """
        """
        A strategy is to read from the end, if we found a #, then we read the two chars before # together,
        otherwise we only read one digit
        """
        """
        Time complexity: O(n) since we loop over the string
        Space complexity: O(n) since we need to store the decrypted string for each number
        """
        s_inv = s[::-1]
        index = 0
        decrypt = []
