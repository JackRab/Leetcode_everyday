...
class Solution:
    def allCellsDistOrder(self, R: int, C: int, r0: int, c0: int) -> List[List[int]]:
        """
        Return the coordinates of all cells in the matrix, sorted by their distance to (r0, c0) from smallest distance to largest distance.
        """
        """
        A brute force idea is loop over every element of the matrix and calculate the distance to (r0, c0) and store as a dict of
        (index, dist), where index is the index in the matrix from a list perspective
        """
        """
